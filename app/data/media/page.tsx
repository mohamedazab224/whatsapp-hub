...
import { Calendar, Eye, Copy, Download, Trash2 } from "lucide-react"
...
export default async function MediaPage() {
  try {
    ...
    const mediaItems = (result as any).data || []
    ...
    logger.error("Error loading media page", { error })
    return (
      <div className="flex h-screen bg-background">
        <Sidebar />
        <main className="flex-1 overflow-y-auto p-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-2xl font-bold">واتساب ميديا</h1>
              <p className="text-sm text-muted-foreground mt-1">
                تصفح المرفقات الإعلامية عبر جميع المحادثات.
              </p>
            </div>
          </div>
          <div className="border rounded-lg bg-card overflow-hidden p-6 text-center text-sm text-muted-foreground">
            تعذر تحميل الوسائط. تأكد من تكوين قاعدة البيانات بشكل صحيح.
          </div>
        </main>
      </div>
    )
